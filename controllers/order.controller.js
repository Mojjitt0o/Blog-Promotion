...
const createOrder = async (req, res) => {
    ...
    try {
        const { items } = req.body;
        if (!items || items.length === 0 || items.some(item => !item.name || item.quantity <= 0)) {
            await transaction.rollback();
            return res.status(400).send(
                `Please fill items with name and quantity greater than 0. Example: { "items": [ {"name": "Item1", "quantity": 10}, {"name": "Item2", "quantity": 15} ] }`
            );
        }
        ...
        const mergedItem = [];
        itemDB.forEach((itemDB) => {
            mergedItem.push({ ...itemDB, ...(items.find(item => itemDB.name === item.name)) });
        });
        ...
        if (isNaN(totalOrderPrice) || isNaN(totalOrderQuantity)) {
            await transaction.rollback();
            return res.status(400).json(
                `Please fill items with name and quantity. Example: { "items": [ { "name": "Item1", "quantity": 10 }, { "name": "Item2", "quantity": 15 } ] }`
            );
        }
